...
RSpec.feature "PostShows", type: :feature do
  ...
  let!(:comment1) { Comment.create(user: user, post:, text: 'first comment') }
  let!(:comment2) { Comment.create(user: user, post:, text: 'second comment') }
  let!(:comment3) { Comment.create(user: user, post:, text: 'third comment') }
  let!(:like1) { Like.create(user: user, post:) }
  ...
  scenario 'see the username and comment of each post' do
    ...
    Comment.create(user: user2, post:, text: 'fifth comment')
    Comment.create(user: user3, post:, text: 'sixth comment')
