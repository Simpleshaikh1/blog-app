...
RSpec.feature "UserShows", type: :feature do
  ...
  context 'show' do
    let(:first_user) { User.first }
    let(:first_user_posts) { first_user.three_most_recent_posts }
    ...
    it 'displays the first three posts and a "Show All" button' do
      ...
      expect(page).to have_content(first_post.text) if first_post
      expect(page).to have_content(second_post.text) if second_post
      expect(page).to have_content(third_post.text) if third_post
      expect(page).to have_selector('a', text: 'See all post')
    end
  end
